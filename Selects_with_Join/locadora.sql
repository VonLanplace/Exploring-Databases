...
CREATE	TABLE	filme(
	...
	ano		INT		NULL		CHECK(ano <= YEAR(GETDATE())),
	PRIMARY	KEY(id)
)
...
FORMAT(lc.data_locacao, 'dd/MM/yyyy') AS data_locacao, 
DATEDIFF(DAY, lc.data_locacao, lc.data_devolucao) AS Qtd_dias_alugado, 
fi.titulo, fi.ano
FROM cliente cl
INNER JOIN locacao lc
  ON cl.num_cadastro = lc.num_cliente
INNER JOIN dvd
  ON lc.num_dvd = dvd.num
INNER JOIN filme fi
  ON dvd.id_filme = fi.id
...
SELECT est.nome, est.nome_completo, fil.titulo
FROM estrela est 
INNER JOIN filme_estrela fisest 
  ON est.id = fisest.id_estrela
INNER JOIN filme fil
  ON fisest.id_filme = fil.id
WHERE fil.ano = 2015;
...
SELECT fil.titulo,
FORMAT(dvd.data_fabricacao, 'dd/MM/yyyy') AS data_fabricacao,
CASE
  WHEN DATEDIFF(YEAR, fil.ano, GETDATE()) > 6
    THEN CAST(DATEDIFF(YEAR, fil.ano, GETDATE()) AS VARCHAR(3)) + ' anos'
  ELSE CAST(DATEDIFF(YEAR, fil.ano, GETDATE()) AS VARCHAR(3))
END AS diferenca
FROM filme fil
INNER JOIN dvd
  ON fil.id = dvd.id_filme;
